...
# 진우석
from SEEGV.models import Ticket, HallSchedule, Hall, HallSeat, Movie, TicketSeat
...
# 나의 예매내역(진우석)
def mycgv_ticket(request:HttpRequest) :
    login = request.session.get("login")

    try: 
        myticket = Ticket.objects.filter(member_no=login).order_by("-ticket_date")
    ...
    paginator = Paginator(myticket,MAX_LIST_CNT)
    ...
    return render(request,'mycgv_ticket.html',context)


def mycgv_ticket_detail(request:HttpRequest) :
    login = request.session.get("login")
    no = request.GET.get("no")
    # Ticket, HallSchedule 테이블 검색 
    try: 
        ######### 예매내역 테이블
        myticket = Ticket.objects.get(ticket_no=no)
        print(myticket)
        ticket_dict = {}
        ticket_dict["ticket_no"] = myticket.ticket_no
        ticket_dict["ticket_date"] = myticket.ticket_date
        ticket_dict["ticket_totalcnt"] = myticket.ticket_totalcnt
        ticket_dict["ticket_totalprice"] = myticket.ticket_totalprice
        if myticket.ticket_cancelflag == 1:
            ticket_dict["ticket_cancelflag"] = '예약취소'
        else:
            ticket_dict["ticket_cancelflag"] = '예약유지'
        
        ########## 상영스케줄 테이블
        myHallSchedule = HallSchedule.objects.get(no = myticket.HallSchedule_no.no)
        ticket_dict["ondate"] =  datetime.strftime(myHallSchedule.ondate, '%Y-%m-%d')
        ticket_dict["stime"] = myHallSchedule.stime

        ########## 영화 테이블
        myMovie = Movie.objects.get(no = myHallSchedule.Movie_no.no)
        ticket_dict["movie_name"] = myMovie.krname

        ########## 상영관 테이블
        myHall = Hall.objects.get(no = myHallSchedule.Hall_no.no)
        ticket_dict["hall_name"] =  myHall.name

        ########## 극장 테이블
        myTheater =Theater.objects.get(no = myHall.Theater_no.no)
        ticket_dict["Theater_name"] =  myTheater.name

        ########## 예매내역별 좌석정보 (다수 데이터 = filter 사용) jjj
        myTicketSeat =TicketSeat.objects.filter(ticket_no = myticket.ticket_no)
        tmp_str = ''
        for i in range(myTicketSeat.__len__()):
            myHallSeat = HallSeat.objects.get(no = myTicketSeat[i].HallSeat_no.no)
            tmp_str += myHallSeat.srow + myHallSeat.scol + ', '
        ticket_dict["TicketSeat_name"] = tmp_str
...
#kkk jjj
    ...
    # try: 
    #     myHallSchedule = HallSchedule.objects.get(no = myticket)
    #     print("myHallSchedule.ondate")
    #     print(myHallSchedule.ondate)
    #     myhs_ondate = myHallSchedule.ondate
    #     myhs_stime = myHallSchedule.stime
    #     myhs_etime = myHallSchedule.etime
    #     myhs_Movie_no = myHallSchedule.Movie_no
    #     myhs_Hall_no = myHallSchedule.Hall_no
    # except Exception as e:
    #     print(e)

    # try: 
    #     myMovie = Movie.objects.get(no = myHallSchedule)
    #     mym_krname = myMovie.krname
    #     print("mym_krname")
    #     print(mym_krname)
    # except Exception as e:
    #     print(e)
    # myticket_dump = json.dumps(myticket_arr)
    context = {
        'login' : login,
        'ticket_dict' : ticket_dict,
    }
    return render(request,'mycgv_ticket_detail.html', context)
