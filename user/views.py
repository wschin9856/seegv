...
# 진우석
from SEEGV.models import Ticket, HallSchedule, Movie
...
def mycgv_ticket_detail(request:HttpRequest) :
    ...
    # jjjj Ticket, HallSchedule 테이블 검색 
    try: 
        myticket = Ticket.objects.filter(member_no=login, ticket_no=no).order_by("-ticket_date")
        myticket_arr = []
        for i in range(myticket.__len__()):
            tmp_dict = {}
            tmp_dict["ticket_totalcnt"] = myticket[i].ticket_totalcnt
            tmp_dict["ticket_totalprice"] = myticket[i].ticket_totalprice
            tmp_dict["ticket_cancelflag"] = myticket[i].ticket_cancelflag
            print("myticket[i].HallSchedule_no")
            print(myticket[i].HallSchedule_no.no)
            print("myticket[i].ticket_totalcnt")
            print(myticket[i].ticket_totalcnt)
            myHallSchedule = HallSchedule.objects.get(no = myticket[i])
            tmp_dict["ondate"] =  myHallSchedule.ondate
            tmp_dict["stime"] = myHallSchedule.stime
            myticket_arr.append(tmp_dict)
    except Exception as e:
        print(e)
...
def mycgv_giftcard(request:HttpRequest) :
    ...
    giftcardUse = GiftcardUse.objects.filter(giftcard_no_id=no)
